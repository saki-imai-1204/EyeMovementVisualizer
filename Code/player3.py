...
class Window(QWidget):
    ...
    def init_ui(self):
        ...
        self.AIBox = QCheckBox('AI')
        self.DriverBox = QCheckBox('Driver')
        self.NavigatorBox = QCheckBox('Navigator')
        self.AIBox.setChecked(False)
        self.DriverBox.setChecked(False)
        self.NavigatorBox.setChecked(False)
        ...
        hboxLayout.addWidget(self.AIBox)
        hboxLayout.addWidget(self.DriverBox)
        hboxLayout.addWidget(self.NavigatorBox)
    ...
    def position_changed(self, position):
        if self.video_selected and self.df_selected:
            ...
            if np.isnan(self.df.at[index, "x_cord"]):
                index = index + 1
            self.fixation.move(
                int((self.df.at[index, "x_cord"]/1920 * self.videowidget.width() + self.videowidget.x() + self.fixation.size().width()//2) + self.offset[0]), 
    ...
    def generate(self):
        ...
        #         return
        #     if time[i] == "-":
        #         start_time = int(time[:i]) + self.start_time
        #         end_time = int(time[i+1:]) + self.start_time
        #         if start_time > end_time:
        #             print(start_time)
        #             print(end_time)
        #             print("Start time should be before end trial time")
        #             return
        #         break
        ...
        if self.AIBox.isChecked():
            type = "AI"
        elif self.NavigatorBox.isChecked():
            type = "Navigator"
        elif self.DriverBox.isChecked():
            type = "Driver"
        else:
        ...
        # filepath = os.path.normpath(os.getcwd() + os.sep + os.pardir) + "/Data/" + part_id + "-" + type + "-" +\
        #           time[:i] + '-' + time[i+1:] + '.csv'
        #fixations.to_csv(filepath + "df.csv")

        #write a csv file
        csv_dest = os.path.normpath(os.getcwd() + os.sep + os.pardir) + "/Data/Dataframes/" + part_id + "-" + type + "-" +\
                  str(int(self.slider2.sliderPosition()[0])) + '-' + str(int(self.slider2.sliderPosition()[1])) + '.csv'
        ...
        self.skipped_df = pd.DataFrame(columns = ["participant_ID", "trial", "time_stamp", "aoi_x", "aoi_y", "aoi_width", "aoi_height", "image", "aoi_image"])
        self.skipped_df.astype(float)
        self.fixated_df = pd.DataFrame(columns = ["participant_ID", "trial", "time_stamp", "duration", "x_cord", "y_cord", "aoi_x", "aoi_y", "aoi_width", "aoi_height", "image", "aoi_image"])
        ...
        for index, row in fixations.iterrows():
            ...
            if not image.isNull():
                ...
                image.save( os.path.normpath(os.getcwd() + os.sep + os.pardir) + "/Data/" + part_id + "/" + part_id + "-" + \
                        str(current_time) + '.jpg', 'jpg')
            else:
                print("Null image")

            #image to text
            #image = part_id + "-" + str(current_time) + '.jpg'
            ...
            aoi_image = et.vscode_draw_aoi(aoi, image, os.path.normpath(os.getcwd() + os.sep + os.pardir) + "/Data/" + part_id + "/")
            #aoi_image.show()
            aoi_image.save( os.path.normpath(os.getcwd() + os.sep + os.pardir) + "/Data/" + part_id + "/aoi-" + part_id + "-" + \
                        str(current_time) + '.jpg')
            aoi_image = os.path.normpath(os.getcwd() + os.sep + os.pardir) + "/Data/" + part_id + "/aoi-" + part_id + "-" + \
                        str(current_time) + '.jpg'
            ...
            if result.empty:
                os.remove((os.getcwd() + os.sep + os.pardir) + "/Data/" + part_id + "/" + part_id + "-" + \
                        str(current_time) + '.jpg')
                os.remove(os.path.normpath(os.getcwd() + os.sep + os.pardir) + "/Data/" + part_id + "/aoi-" + part_id + "-" + \
                        str(current_time) + '.jpg')
                continue

            #if not empty, join the result dataframe with part-ID and trial type
            #result = pd.concat([df, result, df2], axis=1)
            result = pd.concat([result, df2], axis=1)
            
            #if a fixation is in skipped dataframe,
            for df_index, row in self.skipped_df.iterrows():
                if ( (self.skipped_df.loc[df_index, "aoi_x"] == result.loc[0, "aoi_x"]) | (self.skipped_df.loc[df_index, "aoi_y"] == result.loc[0, "aoi_y"]) ) \
                        & (self.skipped_df.loc[df_index, "aoi_width"] == result.loc[0, "aoi_width"]) & (self.skipped_df.loc[df_index, "aoi_height"] == result.loc[0, "aoi_height"]):
            ...
            for aoi_index, aoi_row in aoi.iterrows():
                ...
                if (aoi.loc[aoi_index, "aoi_x"] == result.loc[0, "aoi_x"]) & (aoi.loc[aoi_index, "aoi_y"] == result.loc[0, "aoi_y"]) & \
                    (aoi.loc[aoi_index, "aoi_width"] == result.loc[0, "aoi_width"]) & (aoi.loc[aoi_index, "aoi_height"] == result.loc[0, "aoi_height"]):
                    continue
                else:
                    skipped = False
                    #if the token is already in skipped dataframe then continue
                    for df_index, row in self.skipped_df.iterrows():
                        if ( (self.skipped_df.loc[df_index, "aoi_x"] == aoi.loc[aoi_index, "aoi_x"]) | (self.skipped_df.loc[df_index, "aoi_y"] == aoi.loc[aoi_index, "aoi_y"]) ) \
                            & (self.skipped_df.loc[df_index, "aoi_width"] == aoi.loc[aoi_index, "aoi_width"]) & (self.skipped_df.loc[df_index, "aoi_height"] == aoi.loc[aoi_index, "aoi_height"]):
                            skipped = True
                            break
                    #if not in skipped dataframe then add the token 
                    if skipped == False:
                        skipped_token = aoi.loc[aoi_index:aoi_index, ["aoi_x", "aoi_y", "aoi_width", "aoi_height", "image"]]
        ...
        self.fixated_df.to_csv(os.path.normpath(os.getcwd() + os.sep + os.pardir) + "/Data/" + part_id + "/Dataframes/fixation-" + part_id + "-" + \
                        type + "-" +  str(int(self.slider2.sliderPosition()[0])) + '-' + str(int(self.slider2.sliderPosition()[1])) + "df.csv")
        self.skipped_df.to_csv(os.path.normpath(os.getcwd() + os.sep + os.pardir) + "/Data/" + part_id + "/Dataframes/skipped-" + part_id + "-" + \
                        type + "-" +  str(int(self.slider2.sliderPosition()[0])) + '-' + str(int(self.slider2.sliderPosition()[1])) + "df.csv")

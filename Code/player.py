    ...
    def position_changed(self, position):
        if self.video_selected and self.df_selected:
            ...
            if np.isnan(self.df.at[index, "x_cord"]):
                index = index + 1
            self.fixation.move(int(self.df.at[index, "x_cord"]/1920 * self.videowidget.width() + self.videowidget.x() + self.fixation.size().width()//2), 
        int(self.df.at[index, "y_cord"]/1080 *  self.videowidget.height() + self.videowidget.y() + self.fixation.size().height()//2))
            #update the label
            self.first_handle.setText('First handle: ' + str(self.slider2.value()[0]))
            self.second_handle.setText('Second handle: ' + str(self.slider2.value()[1]))
    ...
    def generate(self):
        ...
        if not image.isNull():
            ...
            image.save( os.path.normpath(os.getcwd() + os.sep + os.pardir) + "/Data/" + textboxValue + "-" + \
                 str(int(self.slider2.sliderPosition()[0])) + '-' + str(self.current_time) + '.jpg', 'jpg')
        else:
            print("Null image")

        #pop up window that asks to select the AOI
        app = db.DrawBox(root, os.path.normpath(os.getcwd() + os.sep + os.pardir) + "/Data/" +textboxValue + "-" + \
            str(int(self.slider2.sliderPosition()[0])) + '-' + str(self.current_time) + '.jpg')
        root.mainloop()

        #write a csv file
        csv_dest = os.path.normpath(os.getcwd() + os.sep + os.pardir) + "/Data/" + textboxValue + '-' + \
             str(int(self.slider2.sliderPosition()[0])) + '-' + str(self.current_time) + '.csv'
